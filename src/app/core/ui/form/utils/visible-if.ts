export const isVisible = (
  visibleIf: { [key: string]: any } | { [key: string]: any }[],
  formValue: string
) => {
  if (!visibleIf) {
    return true;
  ...
  function evaluateVisibleIfForArray(
    visibleIfItems: { [key: string]: any }[]
  ): boolean {
    return visibleIfItems.some(item => evaluateVisibleIfForObject(item));
  }

  function evaluateVisibleIfForObject(visibleIfObject: {
    [key: string]: any;
  }): boolean {
    return Object.keys(visibleIfObject).every(dependentFieldName => {
      const handler = Array.isArray(visibleIfObject[dependentFieldName])
        ? valueMatchesOneOf
        : valueMatches;
